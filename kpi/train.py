...
def train(net,xfm,ifm,device,trainloader,ratio_calculator,epoch,losst,optimizer,len_list):
    ...
    for i, data in enumerate(trainloader):
        inputs = data[0][:,:1,:]
        targets = data[1][:,:1,:] #先尝试data[1]＝data[0]，data[1]保存原ts,label位于最后一个feature
...
# 验证集loss计算，便于主程序通过循环确定val_loss最低的模型
def val(net,xfm,ifm,window,device,ratio_calculator,val_loader,epoch,ep,lossv,vloss,counter,patience,threshold,batchsize,len_list,outp,optimizer):
