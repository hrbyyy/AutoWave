...
def combine_para(lists):
    def my_fun(list1, list2):
        return [str(i) + ',' + str(j) for i in list1 for j in
                list2]  # 采用str形式，后续利用split得到每个元素值，如果此处包装成[i，j],则会变成[[i,j],k]形式

    return reduce(my_fun, lists)


window = 60  # 预测步长,30,40
# hw=10        #288
# num_sim=1  #k_sim中k的数目

repeats = 5
# k = window // 2  # 针对window10
k=7 #point_adjust对应的delay_tolerance  #针对window30+
rate_threshold = 0.1
# p0 = '/mnt/A/PycharmProject/wavelet_rec/kpi/ablation/'
# p0 = p0 + 'temporal_conv/'+'window'+str(window)+'/'
# p0=p0+"RMSprop/learning_rate0.001/"
# p0='/mnt/A/PycharmProject/wavelet_rec/kpi/conv_inception/window60/haar/level2/RMSprop1e-3/'
# p0='/mnt/A/PycharmProject/wavelet_rec/kpi/conv_temporal_freq/window60/haar/level2/RMSprop/learning_rate0.001/'
#p0 = '/mnt/A/PycharmProject/wavelet_rec/kpi/ablation/temporal_conv/window10/RMSprop/learning_rate0.001/'
...
def seqlevel_threshold(seqas_array, val_label, outp,
                       score_indicator):  # 定义函数求threshold      传入VN2和VA的anomaly score(合为一个列表),以及val_y(list,真实0，1label,而非regression值)
...
l = window // 2
selected_ts = [16, 26, 27]  # 21,22, 25,28,29,15,17,19,21,12,
...
def post_cal(ts_id, rootp, repeats):
    ...
    for r in range(repeats):
        ...
        val2_seqas = raw_val2as.sum(axis=1).reshape(len(raw_val2as), 1)  # 保持二维形式，便于后续求metric
        test_seqas = raw_testas.sum(axis=1).reshape(len(raw_testas), 1)
      
        # 对于kpi，要先将pointlabel化为seqlabel
