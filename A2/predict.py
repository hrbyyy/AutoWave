...
def inference(device, testloader, nc, c_in, freq_cout, xfm, ifm, window,ratio_nc,  len_list, batchsize,outp, filename):
    ...
    # total_ratio=0
    with torch.no_grad():
        netinf = A2_Reconstructor(nc,window, c_in)
        # net = TemporalConvNet(num_inputs, num_channels)
        # net.to(device)
        netinf=nn.DataParallel(netinf)
        netinf.to(device)
        netinf.eval()
        ratio_calculator = A2_Ratio(window,ratio_nc)
